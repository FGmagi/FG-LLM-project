...
def plot_sequence(df: pd.DataFrame, out_path: Path, filename: str = "data", settings: dict = None):
    """
    ...
    # 1. 温度与湿度（双Y）
    ax0 = axes[0]
    ax0.plot(df['timestamp'], df['air_temp'], label='Temp (°C)', linewidth=1.2, color=palette.get("temp", None))
    ax0.set_ylabel('Air Temp (°C)')
    ax0b = ax0.twinx()
    ax0b.plot(df['timestamp'], df['humidity'], label='RH (%)', linewidth=1.0, linestyle='--', color=palette.get("rh", None))
    ...
    # 2. 降雨（柱状）
    ax1 = axes[1]
    ax1.bar(df['timestamp'], df['rainfall'], width=0.03, label='Rainfall (mm)', color=palette.get("rain", None))
    ...
    # 3. 土壤含水
    ax2 = axes[2]
    ax2.plot(df['timestamp'], df['soil_vwc'], label='Soil VWC (%)', linewidth=1.5, color=palette.get("vwc", None))
    ...
    # 4. 光照
    ax3 = axes[3]
    ax3.plot(df['timestamp'], df['solar_rad'], label='Solar (W/m2)', linewidth=1.2, color=palette.get("solar", None))
    ...
    png_path = out_path / f"{filename}.png"
